...
fn backward_pass_sets_late_dates_and_floats() {
    ...
    for (i, id_opt) in ids.into_iter().enumerate() {
        if let Some(id) = id_opt {
            m.insert(
                id,
                (es.get(i).unwrap(), ef.get(i).unwrap(), ls.get(i).unwrap(), lf.get(i).unwrap(), tf.get(i).unwrap(), crit.get(i).unwrap())
            );
        }
    }
    ...
    assert_eq!(m.get(&4).map(|v| (v.2, v.3)), Some((td(d(2025,1,15)), td(d(2025,1,17)))));
    // T2 on critical path
    assert_eq!(m.get(&2).map(|v| (v.2, v.3, v.4, v.5)), Some((td(d(2025,1,9)), td(d(2025,1,14)), 0, true)));
    // T3 has slack
