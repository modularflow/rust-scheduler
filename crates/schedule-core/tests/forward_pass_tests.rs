...
fn forward_pass_computes_early_dates_across_dag() {
    ...
    assert_eq!(
        m.get(&1).copied(),
        Some((to_days(d(2025, 1, 6)), to_days(d(2025, 1, 8))))
    );
    assert_eq!(
        m.get(&2).copied(),
        Some((to_days(d(2025, 1, 9)), to_days(d(2025, 1, 14))))
    );
    assert_eq!(
        m.get(&3).copied(),
        Some((to_days(d(2025, 1, 9)), to_days(d(2025, 1, 10))))
    );
    assert_eq!(
        m.get(&4).copied(),
        Some((to_days(d(2025, 1, 15)), to_days(d(2025, 1, 17))))
    );
}
