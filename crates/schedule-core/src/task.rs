...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct RationaleTemplateEntry {
    pub name: String,
    pub weight: f64,
    #[serde(default)]
...
pub enum ProgressRationaleTemplate {
    #[serde(rename = "fifty_fifty")]
    FiftyFifty,
    #[serde(rename = "three_phase_balanced")]
    ThreePhaseBalanced,
    Custom(Vec<RationaleTemplateEntry>),
}

impl ProgressRationaleTemplate {
    pub fn key(&self) -> &'static str {
        match self {
            ProgressRationaleTemplate::FiftyFifty => "fifty_fifty",
            ProgressRationaleTemplate::ThreePhaseBalanced => "three_phase_balanced",
            ProgressRationaleTemplate::Custom(_) => "custom",
        }
    }

    pub fn variants() -> &'static [(&'static str, &'static str)] {
        &[
            ("fifty_fifty", "Two evenly-weighted rationale checkpoints"),
            (
                "three_phase_balanced",
                "Three-phase plan (30%, 40%, 30%) suitable for design-build-close flows",
            ),
        ]
    }

    fn entries(&self) -> Vec<RationaleTemplateEntry> {
        match self {
            ProgressRationaleTemplate::FiftyFifty => vec![
                RationaleTemplateEntry::new("Phase A", 0.5),
                RationaleTemplateEntry::new("Phase B", 0.5),
            ],
            ProgressRationaleTemplate::ThreePhaseBalanced => vec![
                RationaleTemplateEntry::new("Phase A", 0.3),
                RationaleTemplateEntry::new("Phase B", 0.4),
                RationaleTemplateEntry::new("Phase C", 0.3),
            ],
            ProgressRationaleTemplate::Custom(entries) => entries.clone(),
        }
    }

    pub fn materialize(&self) -> Vec<RationaleItem> {
        self.entries()
            .into_iter()
            .enumerate()
            .map(|(idx, entry)| {
...
impl FromStr for ProgressRationaleTemplate {
    type Err = ();

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.trim().to_ascii_lowercase().as_str() {
            "fifty_fifty" | "50_50" | "fiftyfifty" => Ok(ProgressRationaleTemplate::FiftyFifty),
            "three_phase_balanced"
            | "threephasebalanced"
            | "balanced_three_phase"
            | "three_phase" => Ok(ProgressRationaleTemplate::ThreePhaseBalanced),
            _ => Err(()),
        }
    }
}
...
impl Task {
    ...
    pub fn to_dataframe_row(&self) -> PolarsResult<DataFrame> {
        ...
        columns
            .push(Self::series_from_date("baseline_finish", self.baseline_finish)?.into_column());
        ...
        columns.push(
            Series::new(
                PlSmallStr::from_static("percent_complete"),
                percent_complete,
            )
            .into_column(),
        );
        ...
        columns.push(
            Series::new(PlSmallStr::from_static("schedule_variance_days"), variance).into_column(),
        );
    ...
    pub fn from_dataframe_row(df: &DataFrame, row_idx: usize) -> PolarsResult<Self> {
        ...
        let resource_allocations = if let Ok(col) = df.column("resource_allocations") {
            if let Ok(ca) = col.str() {
                if let Some(raw) = ca.get(row_idx) {
                    let trimmed = raw.trim();
                    if trimmed.is_empty() {
                        ...
                        serde_json::from_str::<Vec<ResourceAllocation>>(trimmed).map_err(|err| {
                            PolarsError::ComputeError(
                                format!("invalid resource_allocations: {err}").into(),
                            )
                        })?
                    }
                } else {
    ...
    pub fn apply_rationale_template(
        ...
        self.pre_defined_rationale = template.materialize();
        self.percent_complete = None;
        task_validation::validate_task(self)?;
        Ok(())
    }
